...
class WebSocketHandler(tornado.web.RequestHandler):
    ...
    def check_ping_pong(self, since_last_ping: float, since_last_pong: float) -> bool:
        """Callback to deside if the connection is alive.
        Called periodically before sending the next ping, if :py:attr:`~ping_interval`
        is set and non-zero.
...
class WebSocketProtocol13(WebSocketProtocol):
    ...
    def periodic_ping(self) -> None:
        ...
        connection_alive = self.handler.check_ping_pong(since_last_ping, since_last_pong)
