...
class TCPClient:
    ...
    async def connect(
        ...
        if ssl_options is not None:
            if timeout is not None:
                handshake_future: "Future[IOStream]" = stream.start_tls(
                    False, ssl_options=ssl_options, server_hostname=host
                )
                try:
                    stream = await gen.with_timeout(timeout, handshake_future)
                except TimeoutError:
                    ssl_stream = getattr(handshake_future, "_ssl_stream", None)
